...
beforeAll(async () => {
  ...
  const uuid1 = "6654dbab-e7fd-431b-aa45-7afd243974f7";
  ...
  await Invite.insertMany([
    ...
    .then(function () {
      console.log("Invite Data inserted"); // Success
    })
    .catch(function (error) {
      console.log(error); // Failure
    });
...
describe("GET /api/invite/:inviteId", () => {
  test("OK", async () => {
    ...
    const url = `/api/invite/` + "6654dbab-e7fd-431b-aa45-7afd243974f7";
    await request(app)
      .get(url)
      .set({ cookie: cookie })
      .then(async (res) => {
...
describe("POST /api/invite", () => {
  test("OK", async () => {
    ...
    const inviteData = { calendarId: "5cabe64dcf0d4447fa60f5e2" };
  ...
  test("Not a member of a calendar", async () => {
    const loginResponse = await request(app)
      .post(`/api/users/login`)
      .send({ email: "hhh2@hhh.com", password: "123123" });
    ...
    const inviteData = { calendarId: "5cabe64dcf0d4447fa60f5e3" };
